...
from utils import getFeetPoints,compare_vect, compare_and_get_index
...
def get_camera_perspective(img, src_points):
    ...
    return M, M_inv, scale_w,scale_h
...
def plot_points_on_bird_eye_view(frame, humansFeetPoints, M, scale_w, scale_h,d_thresh, ref_dist, bird_points, left , up, sliding_windows,past_people):
        ...
        for i in range(len(humansFeetPoints)):
            if (humansFeetPoints[i][0][0] != -1 and humansFeetPoints[i][0][1] != -1) or (humansFeetPoints[i][1][0] != -1 and humansFeetPoints[i][1][1] != -1):
                ...
                if target_index in sliding_windows.keys():
                    if len(sliding_windows[target_index][0]) == 7:
                        sliding_windows[target_index][0].popleft()
                    if len(sliding_windows[target_index][1]) == 7:
