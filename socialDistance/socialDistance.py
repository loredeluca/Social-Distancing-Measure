...
import argparse
from birdViewFunctions import plot_points_on_bird_eye_view, get_camera_perspective, order_points, plot_distance_circle_nodes, plot_lines_between_person
from utils import *

FILE_NAME = '../video/video3.mp4' #Put file name in the same folder
fps_time = 0
MAX_DIM_BIRD_VIEW = 2 #meters
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video0', type=str, default=0)# to start from webcam
    parser.add_argument('--video', type=str, default=FILE_NAME)
    ...
    params = dict()
    params["model_folder"] = "../openpose/models" # change the path of the 'model' openpose folder
    ...
    while cap.isOpened() :
        ...
        + cv2.CALIB_CB_FAST_CHECK)
            ...
            bird_points, new_left, new_right, new_up, new_down = find_roi(image, four_points, crop)
            ...
            M, M_inv = get_camera_perspective(image, np.float32(np.array(bird_points)))

            # Find scale values
            scale_w, scale_h = find_scale(four_points,M)
        ...
        if len(humansFeetPoints) != 0:
            ...
            warped_pts, bird_image = plot_points_on_bird_eye_view(humansFeetPoints, M, scale_w, scale_h,d_thresh, REFERENCE_DISTANCE, bird_points, new_left,new_up,sliding_windows,past_people)
            if len(warped_pts) != 0:
