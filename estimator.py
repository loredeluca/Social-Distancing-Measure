...
def find_position(foot, position):
    if len(foot) > 1:
        leftfoot = foot[0]
        rightfoot = foot[1]
        #print('sx', leftfoot, 'dx', rightfoot)
        coordX = (leftfoot[0] + rightfoot[0]) / 2
        coordY = (leftfoot[1] + rightfoot[1]) / 2
        #print('x', coordX, 'y', coordY)
        position.append(coordX)
        position.append(coordY)
        #print('position', position)
    elif len(foot) == 1:
        coordX = float(foot[0][0])
        coordY = float(foot[0][1])
        print('footoooo', coordX, coordY)
        position.append(coordX)
        position.append(coordY)
    else:
        position.append('noPosition')
    return position
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                #salva la coordinata del piede sx e dx
                if i==10 or i==13:
                    foot.append(center)
                    #print('human', body, 'body part', i, 'coord', center)

                centers[i] = center
                cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
            print('human',body, 'foot',foot)
            position = find_position(foot, position)
            allFootPoints.append([body, position])
            body += 1
